...
/**
 *
 * This script creates a loop topology and then ping last node from the first one:
 *
 * [10.0.0.1] <-- step --> [10.0.0.2]             <-- step -->            [10.0.0.6]
 *                              |                                             |
 *                             step                                          step
 *                              |                                             |
 *                         [10.0.0.3]<-- step --> [10.0.0.4] <-- step --> [10.0.0.5] <-- step --> [10.0.0.7]
 *
 * ping 10.0.0.7
 *
 * When 1/3 of simulation time has elapsed, one of the nodes and its connections is
 * removed, thereby breaking the topology.  By default, this will result in
...
AodvExample::CreateNodes()
{
    ...
    // Create edges
    NodeContainer node01, node12, node23, node34, node46, node45, node51;
    ...
    node45.Add(nodes.Get(4));
    node45.Add(nodes.Get(5));

    node46.Add(nodes.Get(4));
    node46.Add(nodes.Get(6));

    node51.Add(nodes.Get(5));
    node51.Add(nodes.Get(1));
    ...
    ptp.SetDeviceAttribute("DataRate", StringValue("100Mbps"));
    ptp.SetChannelAttribute("Delay", StringValue("2ms"));

    // Create Devices
    NetDeviceContainer device01, device12, device23, device34, device46, device45, device51; 
}
